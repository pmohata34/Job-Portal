...
export const submitApplication = catchAsyncErrors(async (req, res, next) => {
  ...
  // Adjust allowed formats for resume documents
...
export const getApplicationsForEmployer = catchAsyncErrors(
  async (req, res, next) => {
    const { role } = req.user;
    if (role === "Job Seeker") {
      return next(
...
export const getApplicationsForJobseeker = catchAsyncErrors(
  async (req, res, next) => {
    const { role } = req.user;
    if (role === "Employer") {
      return next(
...
export const deleteApplicationByJobseeker = catchAsyncErrors(
  async (req, res, next) => {
    const { role } = req.user;
    if (role === "Employer") {
      return next(
